...
lemma obligation1 : @InfValPred.LoewnerOrder 𝕜 𝕜² _ _ _ _ A2 A1 := by
  ...
  · simp only [ENNReal.some_eq_coe, ENNReal.coe_le_coe]
    ...
    calc
      ...
      _ = 1/2 * (c * starRingEnd 𝕜 c) - c * starRingEnd 𝕜 c * (@inner 𝕜 𝕜² _ ketP ket0 * @inner 𝕜 𝕜² _ ket0 ketP) := by rw [inner_outerProduct_eq_inner_mul_inner]
      _ = 1/2 * (c * starRingEnd 𝕜 c) - c * starRingEnd 𝕜 c * (1/√2 * (1/√2)) := by rw [inner_ketP_ket0, inner_ket0_ketP]
...
open LinearMap in
noncomputable def A3 : InfValPred 𝕜 𝕜² :=
  let P := (1/2 : 𝕜) • 1 + ketbra1
  {
    P := P
    X := 0
    PisPos := isPositiveSemiDefinite_add_of_isPositiveSemiDefinite
      (isPositiveSemiDefinite_real_smul_of_isPositiveSemiDefinite' (one_div_nonneg.mpr zero_le_two) isPositiveSemiDefinite.one)
      isPositiveSemiDefinite_ketbra1
    XisProj := isProjection.zero
    compPX := zero_comp P
  }

lemma obligation2 : @InfValPred.LoewnerOrder 𝕜 𝕜² _ _ _ _ A3 A1 := by
  unfold InfValPred.LoewnerOrder
  intro x
  unfold InfValPred.inner_app_self
  have h2 : inner 𝕜 (A3.X x) x = 0 := by simp [show A3.X = (0 : 𝕜² →ₗ[𝕜] 𝕜²) by rfl]
  by_cases h1 : inner 𝕜 (A1.X x) x = 0 <;> simp only [h1, h2, if_true, if_false]
  · simp only [ENNReal.some_eq_coe, ENNReal.coe_le_coe]
    apply Subtype.mk_le_mk.mpr
    rw [← sub_nonneg, ← map_sub]
    ...
    obtain ⟨c, hc⟩ := exist_smul_ketP_of_inner_ketbraM_eq_zero x h1
    calc
      inner 𝕜 (A1.P x) x - inner 𝕜 (A3.P x) x
        = inner 𝕜 (ketbraP x) x - inner 𝕜 ((((1/2) • 1 + ketbra1) : 𝕜² →ₗ[𝕜] 𝕜²) x) x := rfl
      _ = inner 𝕜 ((1 - ketbraM) x) x - inner 𝕜 ((((1/2 : 𝕜) • 1 + ketbra1) : 𝕜² →ₗ[𝕜] 𝕜²) x) x := by rw [ketbraP_eq_one_sub_ketbraM]
      _ = inner 𝕜 (x - ketbraM x) x - inner 𝕜 ((1/2 : 𝕜) • x + ketbra1 x) x := rfl
      _ = inner 𝕜 x x - inner 𝕜 (ketbraM x) x - (inner 𝕜 ((1/2 : 𝕜) • x) x + inner 𝕜 (ketbra1 x) x) := by rw [inner_sub_left, inner_add_left]
      _ = inner 𝕜 x x - (inner 𝕜 ((1/2 : 𝕜) • x) x + inner 𝕜 (ketbra1 x) x) := by unfold A1 at h1; simp [h1]
      _ = inner 𝕜 x x - (starRingEnd 𝕜 (1/2) * inner 𝕜 x x + inner 𝕜 (ketbra1 x) x) := by rw [← inner_smul_left]
      _ = inner 𝕜 x x - (1/2 * inner 𝕜 x x + inner 𝕜 (ketbra1 x) x) := by simp [RCLike.conj_ofNat]
      _ = 1/2 * inner 𝕜 x x - inner 𝕜 (ketbra1 x) x := by ring_nf
      _ = 1/2 * inner 𝕜 (c • ketP) (c • ketP) - inner 𝕜 (ketbra1 (c • ketP)) (c • ketP) := by rw [hc]
      _ = 1/2 * inner 𝕜 (c • ketP) (c • ketP) - inner 𝕜 (c • ketbra1 ketP) (c • ketP) := by rw [map_smul]
      _ = 1/2 * (c * (starRingEnd 𝕜 c * @inner 𝕜 𝕜² _ ketP ketP)) - c * (starRingEnd 𝕜 c * @inner 𝕜 𝕜² _ (ketbra1 ketP) ketP) := by simp only [inner_smul_left, inner_smul_right]
      _ = 1/2 * (c * starRingEnd 𝕜 c) - c * starRingEnd 𝕜 c * @inner 𝕜 𝕜² _ (ketbra1 ketP) ketP := by simp only [mul_assoc, inner_ketP_ketP, mul_one]
      _ = 1/2 * (c * starRingEnd 𝕜 c) - c * starRingEnd 𝕜 c * @inner 𝕜 𝕜² _ ((outerProduct 𝕜 ket1 ket1) ketP) ketP := rfl
      _ = 1/2 * (c * starRingEnd 𝕜 c) - c * starRingEnd 𝕜 c * (@inner 𝕜 𝕜² _ ketP ket1 * @inner 𝕜 𝕜² _ ket1 ketP) := by rw [inner_outerProduct_eq_inner_mul_inner]
      _ = 1/2 * (c * starRingEnd 𝕜 c) - c * starRingEnd 𝕜 c * (1/√2 * (1/√2)) := by rw [inner_ketP_ket1, inner_ket1_ketP]
      _ = 1/2 * (c * starRingEnd 𝕜 c) - c * starRingEnd 𝕜 c * (1/2) := by rw [show (1 : 𝕜)/√2 * (1/√2) = 1 / 2 by field_simp [← RCLike.ofReal_mul, RCLike.ofReal_ofNat]]
      _ = 0 := by ring_nf
  · apply le_top
